...
do
    ...
    if [ $CURL_ONLINE = 200 ]; then
        ...
        if [ $CURL_INFO_PHP_PHP = 200 ] || [ $CURL_INFO_PHP_PHP = 200 ] || [ $CURL_PHP_INFO_PHP = 200 ] || [ $CURL_TEST_PHP = 200 ]; then
            mkdir "../core3_${TIME}";
            echo ${FILE}> ../core3_$TIME/about.txt;

            if [ ${CURL_INFO_PHP_PHP} = 200 ]; then
                curl -x "${FILE}/infophp.php" -o "../core3_${TIME}/phpinfo_php.html";
                echo "discover vulnerabilities.";
                echo "core3 discover vulnerabilities. ADDRESS: ${FILE}" >> ./log/log.log;
            else
                echo 'not vulnerable.';
            fi

            if [ ${CURL_INFO_PHP_PHP} = 200 ]; then
                curl -x "${FILE}/info.php" -o "../core3_${TIME}/info_php.html";
                echo "discover vulnerabilities.";
                echo "core3 discover vulnerabilities. ADDRESS: ${FILE}" >> ./log/log.log;
            else
                echo 'not vulnerable.';
            ...
            if [ ${CURL_TEST_PHP} = 200 ]; then
                curl -x "${FILE}/test.php" -o "../core3_${TIME}/testphp.html";
                echo "discover vulnerabilities.";
                echo "core3 discover vulnerabilities. ADDRESS: ${FILE}" >> ./log/log.log;
            else
                echo 'not vulnerable.';
            ...
            echo 'not vulnerable.';
            echo "core3 not vulnerable.           ADDRESS: ${FILE}" >> ./log/log.log;
        fi
    else
        echo 'not site.'
        echo "core3 not site.                 ADDRESS: ${FILE}" >> ./log/log.log;
    fi
done < ./tmp_textdb/x3.txt
